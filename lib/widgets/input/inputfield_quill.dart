...
class InputfieldQuill extends StatefulWidget {
  ...
  String contentToJSON(String content, TextStyle style) {
    ...
    if (style.color != null)
      attributes["color"] = ColorUtil.getHexFromColor(style.color);
...
class InputfieldQuillState extends State<InputfieldQuill> {
  ...
  void changeHeaderContent(String type, Map<String, String> text) {
    ...
    for (int i = 0; i < contentDescriptions.length; i++) {
      ...
      if (insertText != null) {
        finalText +=
            insertText + (i == contentDescriptions.length - 1 ? "" : "\n");
    ...
    blockFocus = true;
    _controller?.replaceText(0, _controller!.document.length - 1, finalText, null,
        ignoreFocus: true);
    blockFocus = false;
  }
    ...
    return IgnorePointer(
        ...
        child: QuillEditor(
            ...
            onTapDown:(details, p1) {
              ...
              if( viewModelMain.currentContainer.value!=ShowingContainer.EditingTool)
            viewModelMain.currentContainer.value=ShowingContainer.EditingTool;
            return false;
            },
            controller: _controller??QuillController.basic(),
