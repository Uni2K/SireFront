...
/// {@end-tool}
@immutable
class InteractiveViewerAdjusted extends StatefulWidget {
  final Offset fixedFocalPoint;
  final double heightParent, heightChild;

  /// Create an InteractiveViewer.
  ///
  /// The [child] parameter must not be null.
  InteractiveViewerAdjusted(
      {Key? key,
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      InteractiveViewerAdjusted.getNearestPointOnLine(
  ...
  @override
  InteractiveViewerAdjustedState createState() =>
      InteractiveViewerAdjustedState();
}

class InteractiveViewerAdjustedState extends State<InteractiveViewerAdjusted>
  ...
  double _currentRotation = 0.0; // Rotation of transformationController.value.
  ...
  void _onScaleStart(ScaleStartDetails details) {
    ...
    _gestureType = null;
    _panAxis = null;
    _scaleStart = transformationController!.value.getMaxScaleOnAxis();
    _referenceFocalPoint = transformationController!.toScene(
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    switch (_gestureType!) {
      case _GestureType.scale:
        ...
        transformationController!.value = _matrixTranslate(
          transformationController!.value,
      ...
      case _GestureType.rotate:
        ...
        transformationController!.value = _matrixRotate(
          transformationController!.value,
  ...
  Offset? savedLocalPositionScene;
  double? savedYCoordinate;

  // Handle mousewheel scroll events.
  void receivedPointerSignal(PointerSignalEvent event, {double? scaleChangeFix}) {
    if (event is PointerScrollEvent) {
      ...
      } else if (event.position.dy != savedYCoordinate) {
        //does not match -> reset
        savedLocalPositionScene = null;
      ...
      if (scaleChangeFix != null) scaleChange = scaleChangeFix;
  ...
  void initState() {
    ...
    transformationController!.addListener(_onTransformationControllerChange);
  ...
  @override
  void didUpdateWidget(InteractiveViewerAdjusted oldWidget) {
  ...
  void dispose() {
    _controller.dispose();
    transformationController!.removeListener(_onTransformationControllerChange);
    if (widget.transformationController == null) {
      transformationController!.dispose();
    }
    super.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    return Listener(
      key: _parentKey,
      onPointerSignal: (_) => receivedPointerSignal(_),
...
/// See also:
///
///  * [InteractiveViewerAdjusted.transformationController] for detailed documentation
...
class TransformationController extends ValueNotifier<Matrix4> {
  ...
  ///       _childWasTappedAt = transformationController.toScene(
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return InteractiveViewerAdjusted.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        InteractiveViewerAdjusted.getNearestPointInside(point, boundary);
